...
def main():
    ...
    try:
        while True:
            ...
            pwm_value = map_user_input(brightness_percent)
            set_rgb_led(pwm_red_right, pwm_green_right, pwm_blue_right, 0, 0, pwm_value)  # Green color
            set_rgb_led(pwm_red_left, pwm_green_left, pwm_blue_left, 0, 0, pwm_value)  # Green color
            sleep(duration)
