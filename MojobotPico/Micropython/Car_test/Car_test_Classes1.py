...
motor1_pwm = PWM(Pin(motor1_pwm_pin))
motor2_pwm = PWM(Pin(motor2_pwm_pin))
...
while True:
    ...
    if distance_cm > 3:
        if tracking_state == 0:
            ...
            tracking_state = sensors.get_tracking()
            motors.motors_speed(-30, 30)  # Turn right
            left_led_pin.off()
            right_led_pin.on()  # Turn on right LED
        elif tracking_state == 1:
            tracking_state = sensors.get_tracking()
            motors.motors_speed(30, -30)  # Turn left
            left_led_pin.on()  # Turn on left LED
            right_led_pin.off()
        elif tracking_state == 11:
            tracking_state
            motors.motors_speed(50, 50)  # Move forward
            left_led_pin.on()  # Turn on left LED
            right_led_pin.on()  # Turn on right LED
    else:
        tracking_state = sensors.get_tracking()
        motors.motors_speed(0,0)  # Stop motors
